...
class SongControllerIT {
  ...
  private SpotifySong addSongWithName(String name) {
    ...
    Model<SpotifySong> songModel = Instancio.of(SpotifySong.class)
            // --- Basic Setup ---
            ...
            .generate(field(SpotifySong::getDanceability), gen -> gen.math().bigDecimal().scale(3).min(BigDecimal.ZERO).max(BigDecimal.ONE))
            // energy DECIMAL(4,3) CHECK (>= 0 AND <= 1)
            .generate(field(SpotifySong::getEnergy), gen -> gen.math().bigDecimal().scale(3).min(BigDecimal.ZERO).max(BigDecimal.ONE))
            ...
            .generate(Select.fields().matching("speechiness|acousticness|liveness|valence"),
                    gen -> gen.math().bigDecimal().scale(5).min(BigDecimal.ZERO).max(BigDecimal.ONE))
            // instrumentalness DECIMAL(10,9) CHECK (>= 0 AND <= 1)
            .generate(field(SpotifySong::getInstrumentalness), gen -> gen.math().bigDecimal().scale(9).min(BigDecimal.ZERO).max(BigDecimal.ONE))
            ...
            .generate(field(SpotifySong::getAlbumReleaseDate), gen -> gen.temporal().localDate().past())
            .generate(field(SpotifySong::getTempo), gen -> gen.doubles().range(80d, 160d).as(BigDecimal::valueOf))
